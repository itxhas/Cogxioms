...
import { FaTools, FaFolderOpen, FaHandshake, FaInfoCircle } from 'react-icons/fa';
import './Header.css';
      ...
      <div className="container">
        ...
        <nav className="nav">
          <ul className="navList">
            {navItems.map((item) => (
              <motion.li key={item.name} className="navItem" whileHover={{ scale: 1.05 }}>
                <NavLink to={item.href} className={({ isActive }) => `navLink${isActive ? ' active' : ''}`}>
                  {item.icon}
                  {item.name}
                </NavLink>
              </motion.li>
        ...
        {!isOpen && (
          <button className="mobileButton" onClick={() => setIsOpen(true)} aria-label="Open menu">
            <FiMenu size={24} />
          </button>
        )}
      </div>
